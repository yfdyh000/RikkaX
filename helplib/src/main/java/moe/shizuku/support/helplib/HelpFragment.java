...
public class HelpFragment extends PreferenceFragment {
    ...
    public void onCreatePreferences(Bundle bundle, String s) {
        addPreferencesFromResource(moe.shizuku.helplib.R.xml.help);
    }
    ...
    public void addPreference(String categoryKey, String title, Drawable icon, Preference.OnPreferenceClickListener listener) {
        Preference preference = new Preference(getContext(), null,moe.shizuku.helplib.R.attr.preferenceStyle, moe.shizuku.helplib.R.style.HelpTheme_Preference);
        preference.setTitle(title);
        preference.setIcon(icon);
    ...
    public void addArticle(@StringRes int title, @RawRes final int res) {
        addPreference(KEY_HELP, title, moe.shizuku.helplib.R.drawable.helplib_document_24dp, new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
                getFragmentManager().beginTransaction()
                        .setCustomAnimations(moe.shizuku.support.design.R.animator.dir_enter, moe.shizuku.support.design.R.animator.dir_leave, moe.shizuku.support.design.R.animator.dir_enter, moe.shizuku.support.design.R.animator.dir_leave)
